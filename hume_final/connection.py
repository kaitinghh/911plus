...
HUME_API_KEY = "o0978jROEEUAA45htWw7BbPAqRP0mcGVUARlO2k1DRP2mTzF"
HUME_SECRET_KEY = os.getenv("HUME_SECRET_KEY")
HUME_CONFIG_ID = "9875ca3d-991b-4704-9301-e4c1a0cd5b8b"
...
class Connection:
    ...
    async def _send_audio_data(
        ...
        while True:
            ...
            # Write the audio data to the buffer using the wave module
            with wave.open(wav_buffer, 'wb') as wf:
                wf.setnchannels(1)  # Mono
                wf.setsampwidth(2)  # 2 bytes for PCM_16
                wf.setframerate(sample_rate)
                wf.writeframes(np_array.tobytes())  # Write the byte data

            # Move the cursor back to the beginning of the BytesIO buffer
            wav_buffer.seek(0)

            # Read the content of the buffer (if needed)
            wav_content = wav_buffer.getvalue()
